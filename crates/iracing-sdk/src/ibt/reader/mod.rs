...
use crate::{
    ByteRegion, IRacingSDKError, IbtLayout, Result, SessionInfoBuffer, VariableHeadersSnapshot,
...
impl IbtReader {
    ...
    pub fn variable_headers_snapshot(&mut self) -> Result<Option<VariableHeadersSnapshot>> {
        if let Some(variable_headers) = self.layout.metadata().variable_headers() {
            ...
            let buffer =
                VariableHeadersSnapshot::try_from_region_bytes(bytes, variable_headers.count())?;
