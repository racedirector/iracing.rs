...
macro_rules! sdk_bitmask {
    (
        ...
        impl $name {
            ...
            ///
            /// An empty mask is always considered present.
            pub const fn has_all(self, mask: Self) -> bool {
                self.0 & mask.0 == mask.0
            }

            /// Returns whether any bits in `mask` are set.
            ///
            /// An empty mask never intersects this value.
            pub const fn has_any(self, mask: Self) -> bool {
                self.0 & mask.0 != 0
            }
        ...
        impl $crate::VarData for $name {
            fn from_bytes(
                ...
                if info.data_type != $crate::irsdk::VariableType::BitField {
...
mod tests {
    ...
    fn bitmask_constructors_and_queries_preserve_unknown_bits() {
        ...
        assert!(mask.has_all(TestBitmask::FIRST));
        assert!(mask.has_all(TestBitmask::empty()));
        assert!(!mask.has_all(TestBitmask::SECOND));
        assert!(mask.has_any(TestBitmask::FIRST));
        assert!(!mask.has_any(TestBitmask::SECOND));
        assert!(!mask.has_any(TestBitmask::empty()));

        let both = TestBitmask::FIRST.union(TestBitmask::SECOND);
