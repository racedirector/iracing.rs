...
mod raw_value_tests {
    ...
    #[test]
    fn session_flags_preserve_unknown_wire_bits() {
        const RAW: u32 = 0x8000_0004;

        let flags = SessionFlags::from_bits_retain(RAW);

        assert_eq!(flags.bits(), RAW);
        assert!(flags.contains(SessionFlags::GREEN));
        assert_eq!(u32::from(flags), RAW);
    }

    #[test]
    fn incident_flags_extract_packed_fields_from_raw_value() {
        const RAW: u32 = 0x8000_0408;

        let flags = IncidentFlags::from(RAW);

        assert_eq!(flags.bits(), RAW);
        assert_eq!(flags.report_bits(), 8);
        assert_eq!(flags.penalty_bits(), 4);
    }

    #[test]
    fn status_field_preserves_raw_i32_bits() {
        let raw = i32::MIN | StatusField::CONNECTED.bits();
        let status = StatusField::from_bits(raw);

        assert_eq!(status.bits(), raw);
        assert!(status.contains(StatusField::CONNECTED));
    }

    #[cfg(feature = "codegen")]
    #[test]
    fn flag_schema_keeps_protocol_metadata() {
        use serde_json::Value;

        let schema = schemars::schema_for!(SessionFlags);
        let object = schema.as_value().as_object().expect("object schema");

        assert_eq!(
            object.get("x-irsdk-kind").and_then(Value::as_str),
            Some("bitflags")
        );
        assert!(object.contains_key("x-irsdk-values"));
        assert!(object.contains_key("x-irsdk-known-mask"));
    }
}
