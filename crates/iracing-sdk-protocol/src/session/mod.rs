...
mod protocol_tests {
    ...
    #[test]
    fn session_info_rejects_malformed_yaml_as_protocol_error() {
        let error = SessionInfo::parse("WeekendInfo: [").expect_err("malformed YAML must fail");

        assert!(matches!(
            error,
            ProtocolError::Parse { ref context, .. } if context == "SessionInfo deserialization"
        ));
    }

    #[test]
    fn session_info_accepts_raw_protocol_yaml() {
        let yaml = r#"
WeekendInfo:
...
"#;
        ...
        let parsed = SessionInfo::parse(yaml).expect("valid protocol YAML");

        assert_eq!(parsed.weekend_info.track_name, "test-track");
        assert_eq!(parsed.weekend_info.track_display_name, "Test Track");
        assert!(parsed.session_info.sessions.is_empty());
    }
}
...
impl SessionInfo {
    ...
    pub fn parse(yaml: &str) -> crate::Result<Self> {
        serde_yaml_ng::from_str(yaml).map_err(|e| crate::ProtocolError::Parse {
