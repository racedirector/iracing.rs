...
mod tests {
    ...
    #[tokio::test]
    async fn replay_search_snapshots_sends_then_waits() {
        let Fixture { events, use_cases } = fixture();

        let result = use_cases
            .replay_search(ReplaySearchMode::NextLap)
            .await
            .expect("replay search should succeed");
        ...
        assert_eq!(
            *events.lock().expect("events mutex poisoned"),
            vec![
                Event::ReplayPositionSnapshot,
                Event::Send(BroadcastCommand::ReplaySearch(ReplaySearchMode::NextLap)),
                Event::ReplayPositionWait {
                    previous: replay_position(10, 1, 2.0),
                    expected: ReplayPositionExpectation::AnyChange,
                    timeout: Duration::from_millis(25),
    ...
    #[tokio::test]
    async fn replay_set_state_sends_command_without_observation() {
        let Fixture { events, use_cases } = fixture();

        use_cases
            .replay_set_state(ReplayStateMode::EraseTape)
            .await
            .expect("replay set state should succeed");

        assert_eq!(
            *events.lock().expect("events mutex poisoned"),
            vec![Event::Send(BroadcastCommand::ReplaySetState(
    ...
    #[tokio::test]
    async fn reload_textures_with_car_idx_sends_specific_car_command() {
        let Fixture { events, use_cases } = fixture();

        use_cases
            .reload_textures(Some(7))
            .await
            .expect("reload with car_idx should succeed");

        assert_eq!(
            *events.lock().expect("events mutex poisoned"),
            vec![Event::Send(BroadcastCommand::ReloadTextures(7))]
        );
    }

    #[tokio::test]
    async fn reload_textures_without_car_idx_sends_all_textures_command() {
        let Fixture { events, use_cases } = fixture();

        use_cases
            .reload_textures(None)
            .await
            .expect("reload all should succeed");
    ...
    #[tokio::test]
    async fn video_capture_sends_expected_command() {
        let Fixture { events, use_cases } = fixture();

        use_cases
            .video_capture(VideoCaptureMode::TriggerScreenShot)
            .await
            .expect("video capture should succeed");

        assert_eq!(
            *events.lock().expect("events mutex poisoned"),
            vec![Event::Send(BroadcastCommand::VideoCapture(
    ...
    #[tokio::test]
    async fn pit_command_stream_with_empty_commands_returns_current_snapshot_without_waiting() {
        let Fixture { events, use_cases } = fixture();

        let result = use_cases
            .pit_command_stream(vec![])
            .await
            .expect("empty pit stream should return current snapshot");

        // Should return the snapshot from pit.pit_service_snapshot() directly
        assert_eq!(result, pit_snapshot(0));
        assert_eq!(
            *events.lock().expect("events mutex poisoned"),
            vec![Event::PitSnapshot]
        );
    }

    #[tokio::test]
    async fn telemetry_command_start_expects_enabled() {
        let events = Arc::new(StdMutex::new(Vec::new()));
        let commands = Arc::new(FakeCommands {
            events: Arc::clone(&events),
            error: StdMutex::new(None),
        ...
        let telemetry = Arc::new(FakeTelemetry {
            events: Arc::clone(&events),
            snapshot: telemetry_snapshot(false, false),
            wait: StdMutex::new(Some(Ok(telemetry_snapshot(true, true)))),
        });
        let disabled = Arc::new(crate::broadcast_app::DisabledObservationPort);
        let use_cases = BroadcastUseCases::new(
            commands,
        ...
        let result = use_cases
            .telemetry_command(TelemetryCommandMode::Start)
            .await
            .expect("start should succeed");

        assert!(result.is_disk_logging_enabled);
        assert_eq!(
            *events.lock().expect("events mutex poisoned"),
            vec![
                Event::TelemetrySnapshot,
                ...
                Event::TelemetryWait {
                    previous: telemetry_snapshot(false, false),
                    expected: TelemetryLoggingExpectation {
    ...
    #[tokio::test]
    async fn pit_command_single_waits_for_change() {
        let Fixture { events, use_cases } = fixture();

        let result = use_cases
            .pit_command(PitCommand::Fuel(10))
            .await
            .expect("pit command should succeed");
        ...
        assert_eq!(
            *events.lock().expect("events mutex poisoned"),
            vec![
                Event::PitSnapshot,
                Event::Send(BroadcastCommand::PitCommand(PitCommand::Fuel(10))),
                Event::PitWait {
                    previous: pit_snapshot(0),
                    expected: PitServiceExpectation::AnyChange,
                    timeout: Duration::from_millis(25),
