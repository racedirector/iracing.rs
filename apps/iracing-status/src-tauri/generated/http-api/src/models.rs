    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing HealthResponse".to_string(),
                    );
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing HealthResponse".to_string(),
                        );
                    }
                }
            }
        ...
        std::result::Result::Ok(HealthResponse {
            status: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "status missing in HealthResponse".to_string())?,
            service: intermediate_rep
                .service
                .into_iter()
                .next()
                .ok_or_else(|| "service missing in HealthResponse".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<HealthResponse>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<HealthResponse>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for HealthResponse - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
            ...
            std::result::Result::Ok(value) => {
                match <HealthResponse as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into HealthResponse - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
