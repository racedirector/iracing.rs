...
impl Sample {
    ...
    /// Gets all values in the sample along with names and descriptions.
    ///
...
impl Header {
    ///
    /// Parses a header from the provided pointer.
    ...
    ///
    /// Indicates whether or not the telemetry header is connected.
    pub fn is_connected(&self) -> bool {
        self.status == 1
    }

    ///
    /// Returns the session info yaml string from the provided
    /// pointer.
    pub fn session_info(
        &self,
        location: usize,
    ) -> Result<SessionDetails, Box<dyn std::error::Error>> {
        let start = (location + self.session_info_offset as usize) as *const u8;
        let size = self.session_info_length as usize;
    ...
    ///
    /// Returns the latest telemetry buffer sample.
...
impl DiskSubHeader {
    const DISK_SUB_HEADER_OFFSET: usize = 112;

    pub unsafe fn parse(from: *const c_void) -> DiskSubHeader {
        let raw_header: *const DiskSubHeader = transmute(from.add(Self::DISK_SUB_HEADER_OFFSET));
        *raw_header
    }

    pub fn start_datetime(&self) -> DateTime<Utc> {
        Utc.timestamp_opt(self.session_start_date as i64, 0)
            .single()
            .unwrap_or_else(|| Utc.timestamp_opt(0, 0).unwrap())
    }

    pub fn session_start_datetime(&self) -> DateTime<Utc> {
        self.start_datetime() + Duration::from_secs_f64(self.session_start_time)
    }

    pub fn session_end_datetime(&self) -> DateTime<Utc> {
        self.start_datetime() + Duration::from_secs_f64(self.session_end_time)
    }
}

impl fmt::Debug for DiskSubHeader {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DiskSubHeader")
            .field("session_start_date_raw", &self.session_start_date)
            .field("session_start_date", &self.start_datetime())
            .field("session_start_time_raw", &self.session_start_time)
            .field("session_start_time", &self.session_start_datetime())
            .field("session_end_time_raw", &self.session_end_time)
            .field("session_end_time", &self.session_end_datetime())
            .field("session_lap_count", &self.session_lap_count)
            .field("session_record_count", &self.session_record_count)
            .finish()
    }
}
