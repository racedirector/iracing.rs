    ...
    pub fn sample(&self, timeout: Duration) -> Result<Sample, Box<dyn Error>> {
        ...
        match signal {
            ...
            0x00 => {
                ...
                self.header.telemetry_sample(self.origin)
            }
...
impl Connection {
    ...
    fn header(&self) -> Header {
        unsafe { Header::parse(self.location) }
    }
    ...
    pub fn telemetry(&self) -> Result<Sample, Box<dyn std::error::Error>> {
        self.header()
            .telemetry_sample(self.location as *const std::ffi::c_void)
    }
    ...
    pub fn blocking(&self) -> IOResult<Blocking> {
        Blocking::new(self.location, self.header())
    }
